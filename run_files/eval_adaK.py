...
from models.adaK_model import Align_adaK
    ...
    with torch.no_grad():
        for iter, sample in enumerate(tqdm(test_loader)):
            ...
            for i in range(len(frames1_list)):
                ...
                pred1, pred2, frame2step_dist, loss_step = model(frames1, frames2, training=False)
...
def eval():
    model = Align_adaK( num_class=cfg.DATASET.NUM_CLASS,
...
def parse_args():
    ...
    args = parser.parse_args([
        '--config', 'configs/eval_adaK_config.yml',
        '--root_path', 'train_logs/csv_logs/adaK',
