...
def batched_step_comparison(step_feats1, step_feats2, pair_labels):
    B = len(step_feats1)
    dists = []
    for batch in range(B):
        ...
        step_similarity = torch.cosine_similarity(step_feat1, step_feat2)
        step_dist = single_step_comparison(step_feat1, step_feat2, pair_label)
        dists.append(step_dist)
        
    return torch.stack(dists, dim=-1)


def single_step_comparison(step_features1, step_features2, pair_label):
    return None


def single_align_loss(frame_features1, step_features2, pair_label):
